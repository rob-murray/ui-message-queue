...
class root.UiMessageQueue
	...
	"use strict"
 	...
 	# default options
	@DEFAULT_DELAY : 1000 # timeout value to display message in milisecs
	@DEFAULT_EMPTY_DISPLAY_MESSAGE : "" # default string to display when no messages in queue
	...
	push: ( message ) =>
		...
		# only trigger processing if not already done so, processing will internally work through the queue
	...
	_processMessageQueue: =>
		...
		if @_messageStack.hasItems()
			...
			# if the one message in the queue is the first addition then display it immediately
			...
			setTimeout ((self) ->
				...
				# if there are items in queue then display the next one; otherwise it must be the empty string
